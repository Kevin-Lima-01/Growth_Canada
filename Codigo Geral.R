...
df <- read_excel("C:/Users/KEVIN/Desktop/Lições FGV/5 Semestre/Projeto Macro/Dados/Poupança.xls")
df <- df %>% slice(-(1:2))
...
df_fin <- df_fin %>%
  ...
  filter(`Country Code` %in% c("AUS", "CAN", "GBR", "OED", "USA"))  

df_fin <- df_fin %>%
  filter(Ano >= 1960) %>%  # 
  mutate(PIB_per_capita = PIB_per_capita / 100)
# Fazendo os plots
cores_paises <- c(
...
ggplot(df_fin %>% filter(`Country Code` %in% c("AUS", "CAN", "GBR", "OED", "USA")), 
  ...
  labs(x = "Ano", y = "Poupança em % PIB",
       title = "Evolução da Poupança em % PIB (1960-2023)",
       color = "País") +
